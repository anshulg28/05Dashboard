...
class Dashboard extends MY_Controller {
    ...
    function updateEvent()
    {
        ...
        $impChanges = array('eventName','eventDescription','eventDate','startTime','endTime','costType',
                        ...
                        'ifMicRequired','ifProjectorRequired','isSpecialEvent');
            ...
            if(isset($post[$key]))
            {
                if($post[$key] != $row)
                {
                    if(myInArray($key,$impChanges))
                    {
                        ...
                        elseif($key == 'ifMicRequired' || $key == 'ifProjectorRequired' || $key == 'isSpecialEvent')
                        {
                            $ynPoll = array(
                                '1' => 'Yes',
                                '2' => 'No'
    ...
    function eventApproved($eventId)
    {
        ...
        echo json_encode($data);
        $logDetails = array(
            'logMessage' => 'Function: eventApproved, User: '.$this->userId.' eventId: '.$eventId,
            'fromWhere' => 'Dashboard',
            'insertedDT' => date('Y-m-d H:i:s')
        );
        $this->dashboard_model->saveDashLogs($logDetails);
                ...
                foreach($newOrgs as $key => $row)
                {
                    ...
                    for($i=0;$i<count($oldIds);$i++)
                    {
                        ...
                        if(isset($allRegis) && myIsArray($allRegis))
                        {
                            ...
                            {
                                $totQty += (int)$regRow['quantity'];
                                $subTot += ((int)$regRow['quantity'] * (int)$regRow['price']);
                            }
                            //$subTot = $subTot - ($totQty * NEW_DOOLALLY_FEE);
                            $oldEveAmts[] = $subTot;
                            $total += $subTot;
                            if($total > 30000)
                            {
    ...
    public function saveClientLog()
    {
        $post = $this->input->post();
        $data = array();
        $details = array(
            'errorMsg' => $post['msg'],
            'errorTrace' => $post['reqStatus'],
            'fromWhere' => 'Dashboard',
            'insertedDT' => date('Y-m-d H:i:s')
        );
