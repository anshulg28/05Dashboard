...
class Mailers extends MY_Controller {
    ...
    public function sendMail($mailType)
    {
        ...
        $data['locArr'] = $this->locations_model->getAllActiveLocations();
        $data['globalStyle'] = $this->dataformatinghtml_library->getGlobalStyleHtml($data);
        $data['globalJs'] = $this->dataformatinghtml_library->getGlobalJsHtml($data);
        $data['headerView'] = $this->dataformatinghtml_library->getHeaderHtml($data);
        $data['footerView'] = $this->dataformatinghtml_library->getFooterHtml($data);
    ...
    {
        $locArr = $this->locations_model->getAllActiveLocations();
        ...
        preg_match_all('/\[[extendexpiryby3]\w+\]/', $tagStr, $offer_array);
        if(myIsMultiArray($offer_array))
        {
            foreach($offer_array as $key => $row)
            {
                foreach($row as $subKey)
                {
                    if($subKey == '[extendexpiryby3]')
                    {
                        $newDate =array("membershipEnd"=> date('Y-m-d', strtotime($mugInfo['mugList'][0]['membershipEnd'].' +3 month')));
                        $this->mugclub_model->extendMemberShip($mugInfo['mugList'][0]['mugId'], $newDate);
                        $mugInfo['mugList'][0]['membershipEnd'] = $newDate['membershipEnd'];
                        $d = date_create($newDate['membershipEnd']);
                        $tagStr = str_replace('[extendexpiryby3]',date_format($d,DATE_MAIL_FORMAT_UI),$tagStr);
                        break;
                    }
                }
                break;
        ...
        preg_match_all('/\[[extendexpiryby6]\w+\]/', $tagStr, $offer_array);
        if(myIsMultiArray($offer_array))
        {
            foreach($offer_array as $key => $row)
            {
                foreach($row as $subKey)
                {
                    if($subKey == '[extendexpiryby6]')
                    {
                        $newDate =array("membershipEnd"=> date('Y-m-d', strtotime($mugInfo['mugList'][0]['membershipEnd'].' +6 month')));
                        $this->mugclub_model->extendMemberShip($mugInfo['mugList'][0]['mugId'], $newDate);
                        $mugInfo['mugList'][0]['membershipEnd'] = $newDate['membershipEnd'];
                        $d = date_create($newDate['membershipEnd']);
                        $tagStr = str_replace('[extendexpiryby6]',date_format($d,DATE_MAIL_FORMAT_UI),$tagStr);
                        break;
                    }
                }
                break;
        ...
        //location wise DO code
        foreach($locArr as $locKey => $locRow)
        {
            $locCode = $locRow['locName'].'_docode';
            preg_match_all('/\[['.$locCode.']\w+\]/', $tagStr, $output_array);
            if(myIsMultiArray($output_array))
            {
                foreach($output_array as $key => $row)
                {
                    foreach($row as $subKey)
                    {
                        if($subKey == '['.$locCode.']')
                        {
                            $breakCode = $this->generateBeerCodeWithLocation($mugInfo['mugList'][0]['mugId'], $locRow['id']);
                            $tagStr = str_replace('['.$locCode.']',$breakCode,$tagStr);
                            break;
                        }
                    }
                    break;
    ...
    public function generateBeerCodeWithLocation($mugId, $locId)
    {
        $allCodes = $this->offers_model->getAllCodes();
        $usedCodes = array();
        $toBeInserted = array();
