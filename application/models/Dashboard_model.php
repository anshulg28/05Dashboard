...
//                    LEFT JOIN locationmaster lm ON em.eventPlace = lm.id
//                    WHERE em.costType != 1 AND em.ifActive = ".ACTIVE." AND em.ifApproved = ".EVENT_APPROVED." AND em.isEventCancel = 0
//                    AND em.ifAutoCreated = 0 AND em.eventId NOT IN ('1131', '1132', '1136') GROUP BY em.userId";

        $query = "SELECT SUBSTRING_INDEX(GROUP_CONCAT(em.eventId), ',', 4) AS 'ids', 
                  GROUP_CONCAT(em.eventName SEPARATOR ';') AS 'eveNames', 
                  GROUP_CONCAT(em.eventPlace SEPARATOR ';') AS 'evePlaces', 
                  GROUP_CONCAT(lm.locName SEPARATOR ';') AS 'locNames', 
                  em.creatorName, 
                  em.creatorEmail, 
                  em.creatorPhone 
                  FROM eventcompletedmaster em LEFT JOIN locationmaster lm ON em.eventPlace = lm.id 
                  WHERE em.costType != 1 
                  AND em.ifActive = ".ACTIVE."
                  AND em.ifApproved = ".EVENT_APPROVED."
                  AND em.isEventCancel = 0 AND em.ifAutoCreated = 0 GROUP BY em.userId";
